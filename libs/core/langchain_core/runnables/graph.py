...
class Graph:
    ...
    def trim_first_node(self) -> None:
        ...
        if first_node and _first_node(self, exclude=[first_node.id]) and len({e for e in self.edges if e.source == first_node.id}) == 1:
    ...
    def trim_last_node(self) -> None:
        ...
        if last_node and _last_node(self, exclude=[last_node.id]) and len({e for e in self.edges if e.target == last_node.id}) == 1:
